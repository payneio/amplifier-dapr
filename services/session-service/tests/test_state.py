...
class TestSaveTranscript:
    ...
    def test_saves_with_correct_key_and_value_count(self) -> None:
        ...
        with patch("session_service.state._save_state") as mock_save:
            save_transcript("sess-123", messages, "http://localhost:3500")
...
class TestLoadTranscript:
    ...
    def test_empty_for_missing(self) -> None:
        ...
        with patch("session_service.state._get_state", return_value=None):
            result = load_transcript("missing-session", "http://localhost:3500")
            assert result == []
    ...
    def test_returns_messages(self) -> None:
        ...
        with patch("session_service.state._get_state", return_value=stored):
            result = load_transcript("sess-456", "http://localhost:3500")
            assert len(result) == 2
