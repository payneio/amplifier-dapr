"""Tests for services/svc-content-browser-tester content — validates ported files and describe.yaml.

Acceptance criteria for task-4: Port content to svc-content-browser-tester.
"""
...
SVC_DIR = REPO_ROOT / "services" / "svc-content-browser-tester"
...
def _load_describe() -> dict:
    ...
    assert DESCRIBE_YAML_PATH.exists(), (
        ...
        "Create services/svc-content-browser-tester/describe.yaml per spec."
    )
...
def test_browser_awareness_md_exists() -> None:
    """browser-awareness.md must be present in content/."""
    target = CONTENT_DIR / "browser-awareness.md"
    assert target.exists(), f"Expected {target} to exist"


def test_browser_guide_md_exists() -> None:
    """browser-guide.md must be present in content/."""
    target = CONTENT_DIR / "browser-guide.md"
    assert target.exists(), f"Expected {target} to exist"
...
def test_describe_yaml_name() -> None:
    """describe.yaml name must be 'svc-content-browser-tester'."""
    data = _load_describe()
    assert data.get("name") == "svc-content-browser-tester", (
        f"Expected name='svc-content-browser-tester', got {data.get('name')!r}"
    )
...
def test_exactly_3_agents() -> None:
    """describe.yaml must declare exactly 3 agents."""
    data = _load_describe()
    agents = data.get("agents", [])
    assert len(agents) == 3, (
        f"Expected exactly 3 agents, got {len(agents)}: "
        f"{[a.get('name') for a in agents]}"
    )


def test_all_expected_agent_names_present() -> None:
    """All 3 expected agent names must appear in the agents list."""
    data = _load_describe()
