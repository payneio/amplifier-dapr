"""
Tests for agents/default.yaml — verifies bash/filesystem/search behaviors are removed.
"""

import yaml
from pathlib import Path

AGENTS_DEFAULT_YAML = Path(__file__).parent.parent / "agents" / "default.yaml"


def load_yaml():
    with open(AGENTS_DEFAULT_YAML) as f:
        return yaml.safe_load(f)


def test_yaml_is_valid():
    """agents/default.yaml must be parseable by PyYAML."""
    data = load_yaml()
    assert data is not None


def test_no_svc_bash_reference():
    """agents/default.yaml must have no reference to svc-bash."""
...
def test_no_svc_search_reference():
    """agents/default.yaml must have no reference to svc-search."""
    content = AGENTS_DEFAULT_YAML.read_text()
...
def test_bash_behavior_removed():
    """The 'bash' behavior block must not exist in agents/default.yaml."""
    data = load_yaml()
    behaviors = data["agent"]["behaviors"]
...
def test_filesystem_behavior_removed():
    """The 'filesystem' behavior block must not exist in agents/default.yaml."""
    data = load_yaml()
    behaviors = data["agent"]["behaviors"]
...
def test_search_behavior_removed():
    """The 'search' behavior block must not exist in agents/default.yaml."""
    data = load_yaml()
    behaviors = data["agent"]["behaviors"]
